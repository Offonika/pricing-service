...
- `codebase-memory-mcp` разрешён только для навигации, архитектурного обзора и impact
  analysis в пределах этого репозитория. Граф — указатель, а не доказательство.
- Для A/B-пилота запускай Codex через `./scripts/codex_cbm_pilot.sh`: wrapper не
  меняет глобальный config и открывает только утверждённые graph tools.
- Перед правкой обязательно открой исходник; после правки проверь поведение тестами.
- Не индексируй `/opt/MM` целиком. Для 1С используй специализированные BSL symbol
  index и BSL MCP, а не этот граф.
