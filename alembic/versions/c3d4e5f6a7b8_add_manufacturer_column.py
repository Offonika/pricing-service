"""add manufacturer column for competitor displays
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("competitor_item") as batch:
        if "Производитель" not in cols:
            batch.add_column(sa.Column("Производитель", sa.String(length=64), nullable=True))
...
def downgrade() -> None:
    ...
    op.execute("DROP VIEW IF EXISTS vw_competitor_display_ru;")
    with op.batch_alter_table("competitor_item") as batch:
        if "Производитель" in cols:
            batch.drop_column("Производитель")

    # Restore view without manufacturer column
    if is_sqlite:
        op.execute("""
            CREATE VIEW vw_competitor_display_ru AS
            SELECT
