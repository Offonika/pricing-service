"""add competitor item catalog tables

Revision ID: f3c1e9d2b7aa
Revises: e4c2d9f4e3b2
...
revision: str = "f3c1e9d2b7aa"
down_revision: str | None = "e4c2d9f4e3b2"
...
def upgrade() -> None:
    ...
    if not insp.has_table("competitor_item"):
        op.create_table(
            "competitor_item",
        ...
        op.create_index(
            op.f("ix_competitor_item_competitor"),
            "competitor_item",
            ["competitor"],
            unique=False,
        )
        op.create_index(
            op.f("ix_competitor_item_is_active"),
            "competitor_item",
            ["is_active"],
            unique=False,
        )
        op.create_index(
            op.f("ix_competitor_item_parsed_brand"),
            "competitor_item",
            ["parsed_brand"],
            unique=False,
        )
        op.create_index(
            op.f("ix_competitor_item_sku_norm"),
            "competitor_item",
            ["sku_norm"],
            unique=False,
        )

    if not insp.has_table("competitor_item_snapshot"):
        op.create_table(
