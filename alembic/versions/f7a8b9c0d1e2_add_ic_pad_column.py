"""add IC pad column to competitor items
...
def upgrade() -> None:
    ...
    with op.batch_alter_table("competitor_item") as batch:
        if "Площадка под IC" not in cols:
            batch.add_column(sa.Column("Площадка под IC", sa.Boolean(), nullable=True))
...
def downgrade() -> None:
    ...
    op.execute("DROP VIEW IF EXISTS vw_competitor_display_ru;")
    with op.batch_alter_table("competitor_item") as batch:
        if "Площадка под IC" in cols:
            batch.drop_column("Площадка под IC")
