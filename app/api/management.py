...
def get_executive_dashboard_sales_period(
    ...
    today = date.today()
    if today.month == 12:
        default_month_end = date(today.year, 12, 31)
    else:
        default_month_end = date(today.year, today.month + 1, 1) - timedelta(days=1)
    requested_to = date_to or default_month_end
    requested_from = date_from or today.replace(day=1)
    if requested_from > requested_to:
        raise HTTPException(status_code=422, detail="date_from must be before date_to")
