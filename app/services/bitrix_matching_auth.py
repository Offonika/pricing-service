...
_TOKEN_ALG = "HS256"
_TOKEN_TYP = "MM-MATCHING"
_TOKEN_SCOPE = "matching"
...
def _ensure_bitrix_enabled(settings: Settings) -> None:
    if not settings.matching_bitrix_enabled:
        raise HTTPException(status_code=403, detail="Bitrix matching app is disabled")
    if not settings.matching_bitrix_session_secret:
        raise HTTPException(
            status_code=500, detail="Bitrix matching session secret is not configured"
        )
...
def ensure_bitrix_launch_allowed(
    *,
    ...
    if normalized_domain not in _normal_set(settings.matching_bitrix_allowed_domains):
        raise HTTPException(status_code=403, detail="Bitrix domain is not allowed")
    if normalized_member_id.lower() not in _normal_set(settings.matching_bitrix_allowed_member_ids):
...
def ensure_bitrix_user_allowed(user_id: str, *, settings: Settings | None = None) -> str:
    ...
    allowed_user_ids = _normal_set(settings.matching_bitrix_allowed_user_ids)
    ...
    *,
    ...
    try:
        with urllib.request.urlopen(
            request, timeout=settings.matching_bitrix_rest_timeout_seconds
        ) as response:
...
def _secret(settings: Settings) -> bytes:
    secret = settings.matching_bitrix_session_secret
    if not secret:
        raise HTTPException(
            status_code=500, detail="Bitrix matching session secret is not configured"
        )
...
def create_matching_session_token(
    *,
    domain: str,
    member_id: str,
    user_id: str,
    ...
    expires_at_ts = issued_at + int(settings.matching_bitrix_session_ttl_seconds)
...
def verify_matching_session_token(
    token: str,
    *,
    settings: Settings | None = None,
    now: int | None = None,
    ...
    try:
        ...
        raise HTTPException(status_code=401, detail="invalid matching session") from exc

    signing_input = f"{header_raw}.{payload_raw}"
    expected_signature = _sign(signing_input, settings=settings)
    if not hmac.compare_digest(signature, expected_signature):
        raise HTTPException(status_code=401, detail="invalid matching session")
    ...
    if expires_at_ts <= current_ts:
        raise HTTPException(status_code=401, detail="matching session expired")
