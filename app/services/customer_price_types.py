...
def customer_price_type_case_guidance(snapshot: Any) -> dict[str, Any] | None:
    reasons = {str(item) for item in snapshot.reasons}
    if "conflicting_price_levels" not in reasons:
        return None
    return {
        "title": "В активных договорах указаны разные уровни типа цены",
        "rules": (
            "Выручка контрагента суммируется по всем договорам. Если в активных "
            "договорах указаны разные ценовые уровни, наши правила запрещают выбирать "
            "уровень автоматически: кейс остаётся в «Сверке данных» до исправления "
            "или подтверждения данных в 1С."
        ),
        "recommended_action": (
            "Определить фактически действующий договор и согласованный с клиентом "
            "ценовой уровень. Старый или технический договор закрыть либо вывести "
            "из вида «С покупателем». Если оба договора рабочие, согласовать единый "
            "уровень с руководителем и специалистом 1С."
        ),
        "expected_price_type": (
            "Единый подтверждённый ценовой уровень для всех действующих договоров. "
            "Конкретный тип определяется после сверки и не выбирается автоматически."
        ),
        "manager_attention": [
            "Проверить назначение и актуальность каждого договора.",
            "Уточнить, по какому договору сейчас идут отгрузки и расчёты.",
            "Проверить, не является ли один из договоров старым, техническим или карточным.",
            "Сверить ценовой уровень с договорённостью с клиентом и решением руководителя.",
            "Не выбирать уровень только по общей сумме выручки.",
            "После исправления в 1С проверить, что новый расчёт видит один ценовой уровень.",
        ],
    }
