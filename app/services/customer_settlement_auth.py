...
def _keys(settings: Settings) -> dict[str, bytes]:
    ...
    if active_kid:
        ...
        result[active_kid] = (
            settings.customer_settlements_assertion_active_secret.encode("utf-8")
        )
    if previous_kid:
        ...
        result[previous_kid] = (
            settings.customer_settlements_assertion_previous_secret.encode("utf-8")
        )
    if not result:
...
def _integer_claim(payload: dict[str, Any], name: str) -> int:
    ...
    if isinstance(value, bool) or not isinstance(value, int):
        raise CustomerSettlementAuthError("invalid assertion")
    return value
...
def verify_and_consume_customer_settlement_assertion(
    ...
    *,
    ...
    try:
        ...
        raise CustomerSettlementAuthError("invalid assertion") from exc
