...
def sync_assembly_queue(
    ...
    *,
    ...
    payloads = _fetch_executing_deals(client, limit=limit, now=sync_at)
    existing = {
...
def _fetch_executing_deals(
    ...
    *,
    ...
    cutoff = _ensure_aware(now) - timedelta(days=ASSEMBLY_LOOKBACK_DAYS)
    starts = range(0, limit + BITRIX_PAGE_SIZE, BITRIX_PAGE_SIZE)
    commands = {
        f"page_{start}": "crm.deal.list?"
        + urllib.parse.urlencode(
            _flatten_params(
                {
                    "filter": {
                        "=STAGE_ID": ASSEMBLY_STAGE_ID,
                        f"!{CRM_ORDER_NUMBER_FIELD}": "",
                        ">=MOVED_TIME": cutoff.isoformat(),
                    },
    ...
    try:
        response = client.call("batch", {"halt": 1, "cmd": commands})
    ...
    # A flat list keeps small test doubles and older internal clients compatible.
    if isinstance(payload, list):
        pages = [payload]
    elif isinstance(payload, dict):
        errors = payload.get("result_error") or {}
        if errors:
            raise AssemblyQueueUpstreamError("batch_result_error")
        batch_results = payload.get("result")
        if not isinstance(batch_results, dict):
            raise AssemblyQueuePayloadError("batch result is not a mapping")
        pages = [batch_results.get(f"page_{start}") for start in starts]
    else:
        raise AssemblyQueuePayloadError("batch result is invalid")
    ...
    for page in pages:
        ...
        for row in page:
            raw_deal_id = _clean_string(row.get("ID"))
            if raw_deal_id and raw_deal_id in seen_deal_ids:
                continue
            if raw_deal_id:
                seen_deal_ids.add(raw_deal_id)
            result.append(row)
        if len(result) > limit:
