...
_SAFE_MAPPING_ERROR_RE = re.compile(r"crm_mapping_[a-z0-9_]{1,80}")
...
def _safe_mapping_failure_reason(exc: Exception) -> str:
    if isinstance(exc, CustomerSettlementMappingSourceError):
        reason = str(exc).strip()
        if _SAFE_MAPPING_ERROR_RE.fullmatch(reason):
            return reason
    return "mapping_sync_failed"
...
def run_customer_settlement_mapping_sync(
    *,
    ...
    try:
        ...
        _rollback_quietly(session)
        failure_reason = _safe_mapping_failure_reason(exc)
        try:
            mark_mapping_revision_failed(
                session,
            ...
            _rollback_quietly(session)
        return {"status": "error", "reason": failure_reason}
    finally:
