...
### 4.5. Списания дебиторской и кредиторской задолженности для ОПиУ

Для строк `Прочие доходы / расходы` используется только документ
`КорректировкаДолга` с видом операции `СписаниеЗадолженности`. Общие движения
регистра взаиморасчётов не подходят: в них также находятся продажи, возвраты,
оплаты, переносы долга и взаимозачёты.

| Бизнес-поле | SQL-источник | Правило |
| --- | --- | --- |
| Документ | `_Document160` | Только `_Posted = 0x01`, `_Marked = 0x00` |
| Вид операции | `_Document160._Fld3161RRef -> _Enum276._IDRRef` | Только `_EnumOrder = 2` (`СписаниеЗадолженности`) |
| Организация | `_Document160._Fld3154RRef -> _Reference66._IDRRef` | Только `MASTER MOBILE` |
| Строки долга | `_Document160_VT3169` | Связь по `_Document160_IDRRef` |
| Сумма | `_Document160_VT3169._Fld3173` | Положительная сумма документа |
| Вид задолженности | `_Document160_VT3169._Fld3177RRef -> _Enum258._IDRRef` | `_EnumOrder = 0` — дебиторская, `_EnumOrder = 1` — кредиторская |
| Договор | `_Document160_VT3169._Fld3171RRef -> _Reference37._IDRRef` | Владелец договора определяет контрагента |
| Контрагент | `_Reference37._OwnerIDRRef -> _Reference54._IDRRef` | Используется для трассировки строки |

Классификация в управленческой ОПиУ:

- списание дебиторской задолженности — прочий расход, знак в витрине отрицательный;
- списание кредиторской задолженности — прочий доход, знак в витрине положительный;
- перенос задолженности и взаимозачёт не включаются;
- результат публикуется отдельным месячным слоем
  `profit_loss_debt_adjustments` с SHA256 и статусом `ready`/`partial`.

### 4.6. Физические лица сотрудников и статус увольнения
