...
| Точка самовывоза | `_Document132._Fld10203RRef -> _Reference68._IDRRef` | Высокая | Новый реквизит `Документ.ЗаказПокупателя.осиТочкаСамовывоза`; код и наименование читаются из `_Reference68` |

## 5. Как Определять Склад Получателя

Для новых заказов сайта склад получателя определяется прежде всего по отдельному
реквизиту `осиТочкаСамовывоза`. Ссылка и код подразделения должны точно совпасть с
`external_id` или `code` одной записи в `payload.onec_departments` активного
`logistics_warehouse`. Если заполненное значение неизвестно или совпало с несколькими
складами, документ уходит в `manual_review`; адресный fallback в этом случае запрещен.

Для старых заказов, где новый реквизит пуст, сохраняется прежнее правило по адресу:
...
### 6.1. Полная сборка заказа при нескольких РТУ

Готовность отдельной РТУ не равна готовности всего заказа. Reconciliation обязан
сравнить строки исходного заказа `_Document132_VT2427` с суммой строк всех
проведённых и собранных РТУ `_Document203_VT4966`, связанных с этим заказом.
Сопоставление выполняется по ссылке товара, а сравнение — по количеству.

- `complete`: по каждому товару собрано ровно заказанное количество;
- `partial`: хотя бы одного количества не хватает — CRM остаётся в `EXECUTING`;
- `conflict`: имеется лишнее количество, отменённая/возвращённая часть либо
  несовместимое распределение — только ручная проверка;
- `unavailable`: состав заказа надёжно не прочитан — переход запрещён.

Таким образом, будущая РТУ, которая ещё не создана, всё равно учитывается через
непокрытый остаток исходного заказа. Переход в `FINAL_INVOICE` разрешён только
для `complete` и после записи/readback поля полной сборки в Bitrix.

Если 1С позднее начнёт публиковать расширенное событие, его контракт должен
содержать: номер заказа сайта, ID и номер РТУ, время сборки, признак проведения и
пометки удаления, строки `product_ref + quantity` и стабильный `event_id`.
Событие сообщает учётный факт сборки и не объявляет физическую посылку или трек.
Несколько РТУ по умолчанию могут быть объединены в одну физическую отгрузку.
