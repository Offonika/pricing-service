# Технические требования: загрузка прайс-листов конкурентов

## 1. Источники и файлы
- Активный транспорт — HTTPS с прямыми ссылками на датированные XLSX. Просмотр
  каталога не требуется; URL задаётся шаблоном с `{date}`.
- FTP-транспорт и runtime-реквизиты удалены после успешного scheduled-run
  HTTPS-контура. Имена таблиц `competitor_ftp_*` временно сохранены как совместимый
  контракт с действующим matching.
- Каталоги и маски:
