...
3) **(Опционально, второй этап) Признаки из названия + производитель:**
   - Не считаем совпадение голой строки надежным матчем. Сначала разбираем
     `name` на признаки: тип запчасти, бренд, модель, variant, качество, цвет,
     рамка, партномер и совместимость.
   - Требуем совпадение производителя (`id_proizvod` ↔ бренд из названия
     конкурента, если сможем маппить бренды).
   - Fuzzy/contains по `naim/naim_long` vs `name` можно использовать только как
     вспомогательный сигнал после совпадения ключевых признаков.
...
## 3.1. Уровни уверенности

Один и тот же разбор конкурента может использоваться по-разному:

| Уровень | Минимальное совпадение | Что разрешено |
| --- | --- | --- |
| `market_signal` - рыночный сигнал | модель телефона + тип запчасти | Подтвердить, что рынок по модели живой |
| `demand_signal` - сигнал спроса | модель + тип запчасти + качество/тип | Учитывать в прогнозе спроса |
| `analog_candidate` - кандидат в аналог | модель + тип запчасти + качество + рамка/цвет/партномер или подтвержденная совместимость | Показать закупщику как возможный аналог |
| `accepted_match` - принятый матч | точный код или высокий confidence score с объяснимыми совпавшими признаками | Использовать как сопоставление конкурентной цены с нашим SKU |
| `manual_review` - ручная проверка | неполные признаки, конфликт или низкий confidence score | Не писать в итоговую цену без решения человека |

Правило безопасности: слабое совпадение может подтверждать наличие запчасти на
рынке, но не может выбирать конкретный товар в заказ. Сильное совпадение дает
кандидата в аналог, а спорное совпадение всегда остается в ручной проверке.
...
- Источник бренда из 1С — `id_proизовod`. Если потребуется брендовое сравнение, заведём словарь `id_proизвод → brand_name` и парсер бренда из `name` конкурента.
