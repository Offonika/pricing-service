...
1. **Импортирует данные**:
   ...
   - сигналы по рынку смартфонов: новые модели из пресс-релизов брендов, ключевые фразы по запчастям и поисковый спрос по ним из официального Wordstat API.
...
- `DemandStat`
  - поисковый спрос по фразам из Wordstat API (частотность/регион/дата), хранит историю запросов.
...
#### Генерация ключевых фраз и Wordstat API
...
- Интеграция с Wordstat API:
  - используется официальный Wordstat API в составе Yandex Search API, без скрейпинга интерфейса;
  - сервис хранит API-ключ и `folderId` только в окружении, принимает список фраз и регион, возвращает частотность запросов за последние 30 дней;
  - Яндекс.Директ остаётся отдельным fallback-провайдером;
...
**Назначение:** получать новые модели смартфонов, генерировать поисковые фразы по запчастям и собирать частотность через Wordstat API как отдельный ограниченный источник данных для pricing-engine.
...
- `DemandService` — фасад для вызова выбранного провайдера (`WordstatClient` либо fallback `YandexDirectClient`) и сохранения агрегатов спроса.
- `WordstatClient` — клиент официального Yandex Search API с отдельными API-ключом, `folderId`, лимитами и нормализацией устройств; поддерживает текущую частотность `/v2/wordstat/topRequests` и сырую месячную динамику `/v2/wordstat/dynamics` для воспроизводимого shadow/backtest.
...
- `DemandStat`:
  ...
  - метрики: `impressions` (в Wordstat — частотность запросов), `clicks?`, `ctr?`, `bid_metrics?` (дополнительные поля зависят от провайдера);
  - служебные: `source="wordstat" | "yandex_direct"`, `received_at`.

**Поток:** агент → `/api/agents/devices/models` → `PhoneModel` → генерация фраз → `/api/agents/keywords/bulk` (если фразы сгенерированы агентом) или `KeywordGenerationService` → `Keyword` → `DemandService` → Wordstat API → `DemandStat` → pricing-engine.
...
1. **Сбор рынка смартфонов и спроса (агентный контур)**
   ...
   - `DemandService` принимает набор фраз и регион, обращается к Wordstat API и пишет агрегаты поискового спроса в `DemandStat`.
...
- `app/`
  ...
  - `services/`
    ...
    - `market_research/` — модели телефонов, генерация ключей, спрос через Wordstat API и fallback Яндекс.Директа
    - другие сервисы
