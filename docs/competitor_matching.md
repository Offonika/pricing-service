...
## Принцип уровней совпадения

Голое название конкурента не является надежным основанием для автоматического
сопоставления с нашим SKU. Название сначала разбирается на признаки:
`item_type`, бренд, модель, variant, качество, цвет, рамка, партномер и
совместимость. После этого результат используется на разных уровнях:

| Уровень | Минимальные признаки | Разрешенное использование |
| --- | --- | --- |
| `market_signal` - рыночный сигнал | модель телефона + тип запчасти | Подтвердить, что по модели уже есть рынок в РФ |
| `demand_signal` - сигнал спроса | модель + тип запчасти + качество/тип | Учитывать в прогнозе спроса, но не выбирать SKU автоматически |
| `analog_candidate` - кандидат в аналог | модель + тип запчасти + качество + рамка/цвет/партномер или подтвержденная совместимость | Показывать закупщику как возможный аналог |
| `manual_review` - ручная проверка | конфликт или неполные признаки | Не принимать автоматически |

Правило качества: слабое совпадение дает только рыночный сигнал, сильное
совпадение дает кандидата в аналог, спорное совпадение остается в ручной
проверке. Автоматическое `accepted`-сопоставление допускается только при высокой
уверенности и понятном `rationale_json` - объяснении, какие признаки совпали.
...
## Анализ ручной разметки

- Read-only CLI: `./.venv/bin/python -m tasks.analyze_manual_matching_feedback --json`.
- Для проверки без создания локальных файлов добавить `--no-files`.
- Обычный запуск создаёт Markdown, JSON и очищенный CSV в
  `reports/manual_matching_feedback/`.
- В выборке остаётся последнее решение для пары `Product` / `CompetitorItem`:
  `accept` становится положительной меткой, `reject` — отрицательной, а `revoke`
  исключается. Принятия с конфликтами текущих guardrails или объяснимых проверок
  дисплеев и аккумуляторов выносятся в отдельный список для перепроверки.
- Отчёт показывает покрытие причин, разрезы по категориям и конкурентам, reject,
  объяснимые текущими правилами, и reject без найденного конфликта.
- Для аккумуляторов отдельно диагностируются несовместимый тип товара, партномер,
  ёмкость и уровень `Premium`. Несовпадение уровня качества блокирует только
  автоматическое принятие по партномеру: пара остаётся доступна менеджеру и не
