...
Read-only отбор сотруднического пилота от `2026-08-11` дал следующий результат:

- полностью прочитаны `50 035` CRM-строк;
- в УТ найдено `34` действующих employee-контрагента с допустимыми RUB-договорами;
- точную однозначную связь с пользовательским кабинетом имеют только `8` сотрудников;
- все `8` текущих остатков имеют состояние `debt`; `advance/zero` среди этой
  безопасной выборки отсутствуют;
- importer dry-run успешно проверил `8/8` строк при `inn_control_count=0`;
- после rollback все пять settlement-счётчиков в shadow-БД равны нулю;
- точные hashes dry-run сохранены только в защищённом локальном файле вне репозитория.

Readiness gate закрыт: подтверждённая цель — 10 сотрудников, а точных связей сейчас
только 8. Нельзя дополнять набор бывшими сотрудниками или внешними клиентами без
нового решения. Допустимые варианты — пилот на 8 сотрудниках либо создание двух
дополнительных тестовых кабинетов отдельной задачей.
...
Bootstrap запускать только после устранения дефицита двух сотруднических кабинетов
и включения утверждённого whitelist. Допустимый результат перед первым sync:
