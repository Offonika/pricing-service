...
## Итог 72 часов и production gate 2026-08-29

Контрольные точки `24/48/72h` завершились одинаково успешно: readiness `36/36`,
полная сверка `28 729/28 729`, расхождений `0`, максимальная разница `0,00 RUB`.
Financial revisions активировались атомарно, частичных срезов и ambiguous entries
не было. После 72 часов cron штатно перемещён в `.expired`.

В почасовом CRM mapping журнале было `17` отказов внешнего source и один переход
mapping freshness в `critical`; старый целостный mapping сохранился и последующий
запуск восстановил `ok`. Для production добавляются до трёх коротких попыток на
transient transport/REST отказ и безопасный классификационный код причины. Полный
job по-прежнему имеет только один отложенный повтор и не активирует неполный read.

Staging release нельзя переключать в production напрямую: он использует отдельный
mutable root и прежнюю production-base. Production candidate строится из чистого
integration worktree поверх фактически активного release, с единственным Alembic
head `c9e1f3a5b7d0`. Первое включение production — только `pilot_whitelist`; режим
`all_linked` остаётся закрытым до свежей сверки, письменной приёмки и отдельного
go/no-go.
...
- 2026-08-29 — 72-часовой all-linked shadow-run завершён: три checkpoint дали
  `36/36`, `28 729/28 729`, mismatch `0`; cron штатно истёк. Начата сборка
  production candidate на актуальной базе с merge revision и bounded CRM retry;
  production rollout ограничен pilot whitelist и отдельными гейтами.
