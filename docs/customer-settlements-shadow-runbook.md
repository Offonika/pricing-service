...
mapping/financial revision и balances. Старый CSV не используется: у 9 из 10 его
строк отсутствует валидный ИНН для обязательного identity control.

Новая кандидатная десятка отобрана read-only из полного CRM-набора и УТ:

- из `50 035` CRM-строк найдено `191` однозначное соответствие кабинета
  действующему RUB-контрагенту с валидным ИНН;
- состав кандидатов: `4 debt`, `3 advance`, `3 zero`;
- один прежний пилот сохранён, девять замен требуют отдельного подтверждения до
  apply и включения whitelist;
- live dry-run от `2026-08-11` успешно проверил `10/10` строк и `10` уникальных
  контрагентов; после rollback все пять settlement-счётчиков в shadow-БД равны нулю;
...
Bootstrap запускать после отдельного подтверждения новой десятки и включения её
whitelist. Допустимый результат перед первым sync: `status=ready`, 10 пилотов,
