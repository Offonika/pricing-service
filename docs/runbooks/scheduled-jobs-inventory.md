...
не означает, что запрос безопасен для дневного выполнения. Полные сканирования,
массовые агрегации, исторические сверки и запросы, не подтверждённые
production-canary как лёгкие, разрешены только в ночном окне `00:00–06:00 МСК`.

Вне ночного окна разрешены только ограниченные инкрементальные чтения по
...
часть выносится в отдельный ночной запуск. Неподтверждённо лёгкий запрос считается
тяжёлым и в дневное расписание не допускается.

В первую очередь правило применяется к
`order_fulfillment_sync.query_rtu_signal_by_orders` и полным обновлениям
классификации ассортимента. Их дневные контуры обязаны использовать только
ограниченный инкрементальный режим.
...
- 2026-08-31 — тяжёлые запросы к production `Ekama` ограничены ночным окном
  `00:00–06:00 МСК`; дневные scheduled jobs обязаны использовать только
  ограниченное инкрементальное чтение.
