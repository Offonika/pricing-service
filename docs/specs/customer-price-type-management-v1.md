...
- Несколько договоров с одним уровнем типа цены, включая варианты `бн` и `USD`,
  не блокируют расчёт и не разделяют выручку на отдельные кейсы.
- Если хотя бы в одном действующем договоре выбран распознанный тип цены, расчёт
  выполняется по такому полноценному договору. Другие договоры без типа цены, с
  помеченным или нераспознанным типом не создают `data_check` только по этой
  причине.
...
2. Рабочие очереди и фильтры; пользовательское название фильтра `data_check` —
   «Требуют проверки», поиск работает только внутри actionable-кейсов текущего
   списка по коду или имени. При наведении на карточку очереди интерфейс кратко
   объясняет причину попадания клиента в эту группу.
