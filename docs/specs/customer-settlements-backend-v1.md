...
- изменение `УТ 10.3` или CRM;
- массовая активация на production без ограниченного пилота, свежей сверки и
  отдельного go/no-go;
...
- [x] Пройден новый 72-часовой shadow-run на `crm_readonly`: три контрольные
  точки дали `36/36`, `28 729/28 729` и максимальное расхождение `0,00 RUB`.
- [ ] Получена письменная бухгалтерская приёмка production-пилота.
...
- каждый transient HTTP/Bitrix REST отказ получает до трёх коротких попыток;
- после неуспеха полного mapping job выполняется один повтор через 600 секунд;
- в журнал и failed revision попадает только безопасный классификационный код,
  без URL, токена, ответа CRM и клиентских идентификаторов;
...
- Точечный data-quality hold задаётся только SHA-256 от канонического
  `counterparty_ref`: сырой ref не хранится в env/логах, а исключённая связь не
  попадает ни в mapping revision, ни в access scope. Невалидный hash блокирует sync.
...
14. Объединить settlement-ветку с фактически активным production release и свести
    Alembic-граф отдельной no-op merge revision.
15. Собрать candidate только штатным production release-контроллером и проверить
    clean source, текущую production-base, один migration head и rollback.
16. На production сначала включить `pilot_whitelist`, не `all_linked`; расширение
    разрешается только после свежей сверки, письменной приёмки и окна наблюдения.
...
- 2026-08-29 — начата подготовка controlled production rollout: settlement-код
  объединён с текущей production-base, Alembic-ветки сведены revision
  `c9e1f3a5b7d0`; CRM transport получает bounded retry и безопасные коды причин.
  Массовый `all_linked` cutover запрещён до ограниченного пилота и приёмки.
