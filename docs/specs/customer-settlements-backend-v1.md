...
- [ ] Отобраны и сверены 10 сотрудников с однозначным идентификатором Bitrix–1С,
  затем применены их mapping/whitelist.
- [x] Read-only отбор нашёл 8 однозначно связанных действующих сотрудников;
  importer dry-run прошёл `8/8` без ИНН и без записи в shadow-БД.
...
Readiness gate нового пилотного набора остаётся закрытым до отбора и независимой
сверки всех 10 сотрудников с
`Ведомостью по взаиморасчётам с контрагентами` на одинаковый `as_of`.

## Employee pilot validation 2026-08-11
...
- [ ] Отбор 10 сотрудников, apply mapping/whitelist и бухгалтерская сверка
  сотруднического пилота.
- [x] Технический employee dry-run `8/8`; activation gate закрыт из-за дефицита
  двух однозначно связанных кабинетов.
