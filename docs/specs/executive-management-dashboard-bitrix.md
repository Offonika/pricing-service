...
неоперационные движения. Начисленные УСН, торговый сбор и страховые взносы
читаются из отдельного месячного BP OData-контракта. НДС, удержанный НДФЛ, ЕНС и
оплаты налогов в налоговый расход не входят. Налоги суммируются по месячным
контрактам за весь выбранный период. При `partial`, отсутствующем или отклонённом
контракте расчёт всё равно выполняется по доступной наблюдаемой сумме, а при её
отсутствии — с временным налогом `0`; результат явно помечается предварительным.
Карточка `Чистая прибыль` считается как прибыль до налогообложения минус
начисленные налоги БП. Карточка `Рентабельность чистой прибыли` считается как
чистая прибыль / выручка. При неполных источниках обе карточки сохраняют
расчётное значение и показывают пометку `предварительно`.

Товарные потери загружаются из месячного контракта
`retail-director-monthly/YYYY-MM/retail-director-summary-YYYY-MM.json`. Для
истории с января 2026 используются списания минус оприходования в розничном
контуре магазинов. Операции вне контура магазинов остаются в контроле качества.
Показатель справочный и не уменьшает операционную или чистую прибыль до сверки,
не включены ли списания уже в себестоимость продаж.
