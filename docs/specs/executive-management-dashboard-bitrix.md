...
Для денежных средств дополнительно строится отдельная помесячная сверка.
Параметры `month_from` и `month_to` задают включительный диапазон месяцев и не
меняют выбранный снимок постатейного баланса `month`. Ответ содержит
`available_months`, `selected_month_from`, `selected_month_to` и `cash_monthly`.
Каждая строка `cash_monthly` показывает:

- расчётный остаток на начало месяца;
- валовой приход и расход УТ 10.3, включая внутренние перемещения в обеих
  колонках;
- расчётный конец по формуле `начало + приход − расход`;
- последний доступный фактический cash-position внутри месяца, дату снимка и
  разницу `факт − расчёт`;
- статус источника и признак завершённого календарного месяца.

Расчёт начинается от замороженной строки `cash` начального баланса на
01.01.2026 и последовательно переносит расчётный конец одного месяца в начало
следующего (`cash_turnover_method=opening_snapshot_plus_gross_cashflow`).
Отсутствующий фактический снимок не заменяется расчётным значением. Текущий
незакрытый месяц ограничивается последней датой кэша и явно помечается в
интерфейсе. Это позволяет сверять таблицу со стандартной «Ведомостью по
денежным средствам» УТ 10.3, не маскируя разницу составом касс и банковских
счетов.
