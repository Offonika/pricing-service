...
периода, который уже полностью в прошлом, сервис отдает только факт и не
пересчитывает прогноз (`forecast_status=not_applicable`).
...
Для счётчиков магазинов ниже плана и менеджеров ниже целевой маржи поле
`meta.problem` содержит точный список объектов `{key, label}`. UI строит
проблемные списки только по этому серверному результату и не повторяет правила
плана эвристикой по строкам разреза.
