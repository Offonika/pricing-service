...
Draft-операции v1 строго разделены по ролям: `sender` создаёт и изменяет только
передачу со своего склада, `receiver` — только приёмку на своём складе.
`logist/admin` имеют мониторинг, историю и очередь ошибок, но не создают,
не сканируют и не подтверждают draft; ручного обхода этих проверок нет.
