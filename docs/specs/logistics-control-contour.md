...
## Собственная курьерская доставка клиенту — задача №28

Задача №28 развивается внутри активного контура `pricing-service` как отдельный
сценарий последней мили. Архивный `mastermobile` не реактивируется; его API,
модели и тесты используются только как исходные материалы.

Складские перемещения и курьерская доставка используют общую платформу:
Bitrix-приложение `/bitrix/logistics/`, авторизацию, QR/ручной код, фотографии,
аудит, идемпотентность, web fallback и мониторинг.

State machine не объединяются:

- `logistics_transfer` продолжает вести перемещения между складами;
- для последней мили создаются отдельные сущности доставки, назначения курьера,
  событий и сдачи наличных;
- собственный курьер является активным пользователем приложения, а не пассивным
  `logistics_driver`;
- способ `own_courier` отделяется от `external_carrier`: собственная курьерская
  доставка не направляется на терминал СДЭК/Почты.

Первая очередь включает назначение курьера, `Забрал`, `Отдал`, `Не доставил`,
фото, фактически полученную сумму и монитор сдачи наличных. Курьерский забор
возвратов выполняется отдельной следующей очередью и связывается с задачей №100.
Возвраты через СДЭК/Почту из задачи №3507 остаются отдельным процессом.

Задача №28 является родительской задачей реализации. Технические подзадачи
создаются отдельно для backend, интерфейса Bitrix, интеграции с `1С` и пилотной
приёмки.

### Готовность заказа и связь с задачей №46

Курьерская доставка не создаётся только по наличию CRM-стадии
`FINAL_INVOICE`. Backend требует подтверждённое событие сборки из 1С:

- `СтатусИсполнения = 06 Готов к отправке`;
- код службы доставки `MM_COURIER`;
- заказ не отменён;
- адрес доставки заполнен;
- для предоплаты оплата подтверждена;
- для разрешённой оплаты при получении отсутствие предоплаты не блокирует
  создание доставки.

После проверки backend устанавливает `ready_for_courier_assignment`, создаёт
доставку и передаёт её менеджеру для назначения курьера. При фактической
передаче курьеру CRM переводится из `FINAL_INVOICE` в `IN_DELIVERY`.

Сигнал должен содержать идентификатор заказа, статус исполнения, код службы,
время сборки и уникальный идентификатор события. Повторная доставка одного
события не создаёт вторую доставку.

Задача №46 является зависимой предпосылкой включения автоматического создания
доставок в №3596. Разработка backend может выполняться под feature flag, но
production-пилот №3599 запрещён до production-приёмки единого события сборки.
Подзадача №3598 сохраняет отдельный scope: события после передачи курьеру и
сверка наличных.
...
- 2026-09-03 — задача №28 связана с единым контуром сборки №46:
  `СтатусИсполнения=06` и `MM_COURIER` формируют
  `ready_for_courier_assignment`; одной CRM-стадии для создания доставки
  недостаточно.
- 2026-09-03 — задача №28 зафиксирована как отдельный сценарий собственной курьерской доставки внутри `pricing-service`; складская и курьерская state machine разделены, возвраты курьером вынесены во вторую очередь.
