...
Blueprint `receivable_work` по умолчанию остаётся полностью локальным. Флаг
`--bitrix-readonly` разрешает только чтение типов, стадий, полей, пользователя
Арсена и его подразделений для проверки коллизий; add/update/delete методы
запрещены allowlist-ом и создание процесса требует отдельного утверждения.
