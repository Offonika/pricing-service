...
- уведомление повторяется at-least-once до локального подтверждения успешного вызова.
  `im.notify.personal.add` считается успешным только при строгом положительном ID
  в `result`; пустой или malformed result оставляет доставку pending для retry.
