...
Для обращений сайта поддерживаются два штатных способа ответа: из CRM-карточки
через Agent и непосредственно в исходном тикете под личной учётной записью
сотрудника поддержки. Служебная учётная запись используется только фоновым Agent,
её данные сотрудникам не передаются. Прямые ответы сотрудников и последующие
ответы клиента синхронизируются с той же CRM-карточкой по `source_ticket_id` без
создания дубля. Один ответ отправляется только одним выбранным способом.
...
- 2026-08-30 — утверждены два штатных канала ответа сотрудника: из CRM-карточки и
  непосредственно в исходном тикете; оба направления синхронизируются с одной
  CRM-карточкой без дублей, служебная учётная запись остаётся только для Agent;
