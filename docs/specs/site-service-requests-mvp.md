...
- ежедневно после `11:00 Europe/Moscow` в чат задачи №372 отправляется отдельная
  сводка по всем письмам и обращениям сайта, для которых ещё не подтверждён
  первый ответ; каждое такое обращение повторяется в следующих ежедневных
  сводках до ответа или закрытия и содержит прямые ссылки на карточку CRM и
  исходное обращение;
- если открытых обращений без ответа нет, ежедневная сводка явно сообщает:
  `Просроченных ответов нет`; сведения об интернет-заказах в эту сводку не
  включаются;
