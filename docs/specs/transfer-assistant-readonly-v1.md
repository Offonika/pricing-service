...
Терминологическая граница: внутренний API-статус `pickup_waiting` сохраняется
для обратной совместимости, но в документации и будущем интерфейсе называется
«Расчетный кандидат на самовывоз». Это только read-only предположение по
резерву, размещению, заказу или РТУ. Оно не является физическим состоянием
логистической единицы и не равно CRM-стадии `PICKUP_WAITING` / «Принят
магазином и ожидает клиента», которая возникает только после подтвержденной
приемки на целевой точке.
...
## Граница между перемещением и закупкой

Решением владельца от `2026-08-10` дефицит отдельной точки сначала является
кандидатом на перемещение, а не автоматическим основанием внешней закупки.

Порядок решения:

1. Проверить свободный остаток в других точках, на центральном и транзитных
   складах с учётом резервов и уже начатых перемещений.
2. Если локальную потребность можно закрыть внутри сети, показать кандидат
   перемещения; автоматическое создание документа перемещения не разрешено.
3. В закупку передавать только оставшуюся сетевую потребность, которую нельзя
   закрыть доступным внутренним остатком.
4. Продажу сразу после перемещения считать подтверждением спроса точки, но не
   доказательством отсутствия товара в сети в предыдущие дни.

**Расхождение реализации на `2026-08-10`:** v1 показывает кандидатов
перемещения отдельно и ещё не уменьшает ими расчёт закупки. Связь «кандидат
перемещения → строка внутреннего проекта заказа → остаточная закупка» является
отдельной кодовой задачей; настоящая правка меняет только документацию.
...
- связать кандидатов перемещения с расчётом заказа и вычитать из закупки только
  подтверждённо доступный для перемещения остаток.

## Changelog

- 2026-08-28 — `pickup_waiting` явно отделен от одноименной CRM-стадии: код API сохранен для совместимости, пользовательское название — «Расчетный кандидат на самовывоз».
- 2026-08-10 — локальный дефицит назначен сначала задачей перемещения; внешняя
  закупка должна покрывать только остаточную потребность сети. Зафиксирован
  текущий разрыв между read-only помощником и расчётом заказа.
