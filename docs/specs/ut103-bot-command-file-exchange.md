...
Жизненный статус ассортимента исключён из этого write-контура: он хранится
только в `pricing-service`. Универсальный `nomenclature_property_updates.v1`
сохраняется для других отдельно утверждённых свойств и реквизитов, но отклоняет
восемь legacy lifecycle-полей на стороне Python-экспортера.
...
Экспортёр принимает только разрешённые текущим контуром свойства. Запись
`Статус ассортимента`, причины, источника, даты, утвердившего, профиля
закупочного поведения, ручного минимального остатка и даты пересмотра правила
наличия запрещена независимо от режима `dry_run/apply` и значения `ApprovedBy`.
Исторические result-файлы с такими именами можно читать для аудита, но повторно
отправлять их нельзя.
...
- Статус `applied` с несовпавшей парой readback не считается доказанным
  результатом: lock сохраняется, а worker запускает recovery readback.
- Состояние `dry_run_sent` фиксируется до публикации ready-файла, чтобы сбой
  между атомарным rename и commit восстанавливался тем же MessageId.
...
5. Для разрешённых свойств `nomenclature_property_updates.v1` после тестового `dry_run/apply`
...
- 2026-08-05 - lifecycle-поля исключены из write-контура; универсальный
  property-транспорт сохранён для других отдельно утверждённых сценариев.
