#!/usr/bin/env bash
set -euo pipefail

export TZ="${TZ:-Europe/Moscow}"

REPO_DIR="${REPO_DIR:-/opt/MM/pricing-service-task43-current}"
ENV_FILE="${REPO_DIR}/.env"
ENV_LOADER="${REPO_DIR}/infra/cron/load_env.sh"
LOG_DIR="${LOG_DIR:-/var/log/pricing}"
LOG_FILE="${LOG_FILE:-${LOG_DIR}/logistics_rtu_sync.log}"
PYTHON_BIN="${REPO_DIR}/.venv/bin/python"
LOOKBACK_DAYS="${LOGISTICS_RTU_SYNC_LOOKBACK_DAYS:-14}"
PAGE_SIZE="${LOGISTICS_RTU_SYNC_PAGE_SIZE:-500}"
APPLY="${LOGISTICS_RTU_SYNC_APPLY:-false}"
TIMEOUT_SECONDS="${LOGISTICS_RTU_SYNC_TIMEOUT_SECONDS:-50}"
LOCK_FILE="${LOGISTICS_RTU_SYNC_LOCK_FILE:-/tmp/logistics_rtu_sync.lock}"
SUCCESS_FILE="${LOGISTICS_RTU_SYNC_SUCCESS_FILE:-/var/lib/pricing-service/logistics_rtu_sync.last_success}"

mkdir -p "${LOG_DIR}"
cd "${REPO_DIR}"

exec 9>"${LOCK_FILE}"
if ! flock -n 9; then
  echo "[$(date -Iseconds)] logistics_rtu_sync skipped: previous run is active" >> "${LOG_FILE}"
  exit 0
fi

if [[ -f "${ENV_FILE}" ]]; then
  # shellcheck disable=SC1090
  source "${ENV_LOADER}"
  load_env_file_preserve_json "${ENV_FILE}"
fi

if [[ ! -x "${PYTHON_BIN}" ]]; then
  echo "[$(date -Iseconds)] logistics_rtu_sync failed: release Python is missing" >> "${LOG_FILE}"
  exit 1
fi

DATE_FROM="$(date -u -d "${LOOKBACK_DAYS} days ago" +%F)"
cmd=(
  "${PYTHON_BIN}" -m tasks.sync_logistics_rtu_from_onec
  --date-from "${DATE_FROM}"
  --limit "${PAGE_SIZE}"
)
if [[ "${APPLY}" == "true" ]]; then
  cmd+=(--apply)
fi

started_epoch="$(date +%s)"
echo "[$(date -Iseconds)] starting logistics_rtu_sync apply=${APPLY} date_from=${DATE_FROM} page_size=${PAGE_SIZE}" >> "${LOG_FILE}"
set +e
timeout "${TIMEOUT_SECONDS}" "${cmd[@]}" >> "${LOG_FILE}" 2>&1
status=$?
set -e
duration_seconds="$(( $(date +%s) - started_epoch ))"

if [[ "${status}" -ne 0 ]]; then
  echo "[$(date -Iseconds)] logistics_rtu_sync failed status=${status} duration_seconds=${duration_seconds}" >> "${LOG_FILE}"
  exit "${status}"
fi

mkdir -p "$(dirname "${SUCCESS_FILE}")"
touch "${SUCCESS_FILE}"
echo "[$(date -Iseconds)] logistics_rtu_sync finished duration_seconds=${duration_seconds}" >> "${LOG_FILE}"
