...
if [[ -n "$(git -C "$SOURCE_ROOT" status --porcelain --untracked-files=all)" ]]; then
  echo "source checkout is dirty; release source must have a clean Git tree: $SOURCE_ROOT" >&2
  exit 2
fi
if [[ -e "$FINAL_DIR" ]]; then
...
if [[ -n "$BASE_RELEASE" ]]; then
  ...
  if ! "$PYTHON_BOOTSTRAP" - "$base_release_manifest" <<'PY'
import json
import sys
...
cat >"$TEMP_DIR/release-manifest.json" <<EOF
{
  ...
  "built_at": "$(date -Is)",
  "base_release": "$base_release_value",
