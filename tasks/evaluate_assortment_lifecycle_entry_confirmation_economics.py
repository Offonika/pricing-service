"""Evaluate x1.2 entry e2 + exit d3 on the frozen economic training window.

The task reuses the validated x1.2 entry-e1/exit-d3 economic rows and simulates
only the new immutable entry-e2/exit-d3 trajectory. It never consumes the July
holdout or performs production/external writes.
"""
...
DEFAULT_CANDIDATE_TRAJECTORY_HASH = (
    "e16e89617af5d737d1fd484a0f8abfc72db68b565f2c7eccdfdf8871dbb60e46"
)
...
CANDIDATE_POLICY = "x1.2_entry_e2_exit_d3"
...
def combine_entry_confirmation_results(
    *,
    ...
    if set(baseline_by_level) != set(GROUP_LEVELS):
        raise ValueError("entry_confirmation_reused_baseline_incomplete")
    if set(simulated) != set(GROUP_LEVELS):
        raise ValueError("entry_confirmation_simulated_candidate_incomplete")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if reused["dataset_hash"] != args.dataset_hash:
        raise ValueError("entry_confirmation_reused_dataset_mismatch")
    if reused["holdout_consumed"] is not False:
        raise ValueError("entry_confirmation_reused_holdout_consumed")
    if reused["production_authorized"] is not False:
        raise ValueError("entry_confirmation_reused_production_authorized")
    ...
    if trajectory_row["dataset_hash"] != args.dataset_hash:
        raise ValueError("entry_confirmation_trajectory_dataset_mismatch")
    if trajectory_row["period_to"] < policy_v2.periods.training_to.isoformat():
        raise ValueError("entry_confirmation_trajectory_period_incomplete")
    ...
    checkpoint_path = args.output_dir / "economic-entry-e2-checkpoint.json"
    if checkpoint_path.exists():
        checkpoint = _read_json(checkpoint_path)
        if checkpoint.get("trajectory_hash") != args.candidate_trajectory_hash:
            raise ValueError("entry_confirmation_checkpoint_lineage_mismatch")
    ...
    for index, level in enumerate(GROUP_LEVELS, start=1):
        if level in results:
            print(
                f"entry e2 economics: {index}/{len(GROUP_LEVELS)} {level} reused=1",
                flush=True,
            )
            continue
        ...
        _write_json(
            checkpoint_path,
            {
                "schema": "assortment_lifecycle_entry_confirmation_checkpoint.v1",
        ...
        print(
            f"entry e2 economics: {index}/{len(GROUP_LEVELS)} {level} {metrics}",
            flush=True,
        )
        del result
        gc.collect()

    rows = combine_entry_confirmation_results(reused_rows=reused["results"], simulated=results)
    payload = {
        "schema": "assortment_lifecycle_entry_confirmation_economics.v1",
    ...
    _write_json(args.output_dir / "economic-entry-e2.json", payload)
