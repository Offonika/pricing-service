...
def test_all_linked_worker_replaces_scope_and_rolls_back_failed_change(
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: (_ for _ in ()).throw(
            CustomerSettlementMappingSourceError("crm_mapping_source_rate_limited")
        ),
    )
    source_failed = settlement_workers.run_customer_settlement_mapping_sync(settings=settings)
    assert source_failed == {
        "status": "error",
        "reason": "crm_mapping_source_rate_limited",
    }
    with Session(db_session.get_bind()) as readback:
        failed_revision = readback.scalar(
            select(CustomerSettlementMappingRevision)
            .where(CustomerSettlementMappingRevision.status == "failed")
            .order_by(CustomerSettlementMappingRevision.id.desc())
        )
        assert failed_revision is not None
        assert failed_revision.error_code == "crm_mapping_source_rate_limited"
        assert failed_revision.error_detail == "CustomerSettlementMappingSourceError"
        assert active_pilot_site_user_ids(readback) == ("102",)
...
@pytest.mark.parametrize(
    ("detail", "expected"),
    (
        ("crm_mapping_source_unavailable", "crm_mapping_source_unavailable"),
        ("https://example.invalid/rest/secret", "mapping_sync_failed"),
        ("crm_mapping_source_unavailable?token=secret", "mapping_sync_failed"),
    ),
)
def test_mapping_failure_reason_only_exposes_safe_codes(detail: str, expected: str) -> None:
    assert (
        settlement_workers._safe_mapping_failure_reason(
            CustomerSettlementMappingSourceError(detail)
        )
        == expected
    )
