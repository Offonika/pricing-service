...
def test_export_prices_to_csv(tmp_path: Path):
    rows = [
        ExportRow(article="SKU1", price=100.5, comment="auto"),
        ExportRow(article="SKU2", price=200.0, comment="manual"),
    ]
    output = export_prices_to_csv(rows, tmp_path / "prices.csv")

    assert output.exists()
    content = output.read_text(encoding="utf-8").splitlines()
    assert content[0] == "ARTICLE,PRICE,COMMENT"
...
def test_export_recommendations(tmp_path: Path):
    recs = {
        "SKU1": PricingResult(
            recommended_price=Decimal("110"), floor_price=Decimal("100"), reasons=[]
        ),
        "SKU2": PricingResult(
            recommended_price=Decimal("220"), floor_price=Decimal("200"), reasons=[]
        ),
    }
