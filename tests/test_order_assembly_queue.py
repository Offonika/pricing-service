...
def test_assembly_queue_endpoint_returns_fresh_xml_in_priority_order(monkeypatch) -> None:
    ...
    first_command = calls[0][1]["cmd"]["page_0"]
    assert first_command.startswith("crm.deal.list?")
    assert "filter%5B%3DSTAGE_ID%5D=EXECUTING" in first_command
    assert f"filter%5B%21{service.CRM_ORDER_NUMBER_FIELD}%5D=" in first_command
    assert "filter%5B%3E%3DMOVED_TIME%5D=" in first_command
