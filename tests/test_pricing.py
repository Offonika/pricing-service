...
def test_pricing_uses_floor_when_no_competitors() -> None:
    product = Product(article="SKU", name="Test")
    product.stock = ProductStock(purchase_price=Decimal("100"))

    result = calculate_recommendation(
        product, competitor_min_price=None, min_margin_pct=Decimal("0.2")
    )
...
def test_pricing_uses_market_above_floor() -> None:
    ...
    result = calculate_recommendation(
        product, competitor_min_price=Decimal("150"), min_margin_pct=Decimal("0.1")
    )
...
def test_pricing_uses_floor_if_market_below_floor() -> None:
    ...
    result = calculate_recommendation(
        product, competitor_min_price=Decimal("105"), min_margin_pct=Decimal("0.1")
    )
