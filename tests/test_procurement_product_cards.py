...
def test_product_card_review_keeps_primary_first_and_selects_four_fastest_candidates(
    db_session,
    monkeypatch,
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: {
            ...
            "member_count": 6,
            "members": [
                {"nomenclature_code": code, "name": f"Дисплей {index}"}
                for index, code in enumerate(codes, start=1)
            ],
        },
    )
    ...
    assert all(item["role"] == "candidate" for item in members[1:])
    assert snapshot["family"]["visible_member_count"] == 5
    assert snapshot["family"]["hidden_member_count"] == 1
    assert snapshot["family"]["ranking_source"] == "completed_sales_rate_30_90"


def test_product_card_review_shows_entire_small_family_and_uses_stable_180_fallback(
    db_session,
    monkeypatch,
) -> None:
    codes = ["РБ000000011", "РБ000000012", "РБ000000013", "РБ000000014"]
    sales_180 = ["9", "90", "90", "45"]
    for index, (code, sales) in enumerate(zip(codes, sales_180, strict=True), start=1):
        order = _seed_order(
            db_session,
            ...
            product_code=code,
            product_name=f"Дисплей малой семьи {index}",
        )
        payload = dict(order.lines[0].payload or {})
        payload.update(
