const PROCUREMENT_RISK_LABELS: Record<string, string> = {
  ...
  // Сроки поставки и сезонность: считаются по фактам поставщика, а не по
  // константам, поэтому закупщик должен видеть, откуда взялся горизонт.
  adaptive_lead_time_applied: "Срок поставки взят по фактам поставщика",
  adaptive_lead_time_sync_ready: "Срок поставки по фактам учтён в расчёте заказа",
  ...
  recent_seasonality_adjustment_applied: "Срок поставки поправлен на сезонность",
